<template>
  ...
  <div :class="['sidebar', { 'slide-in': isSidebarOpen }]">
    ...
    <p class="all-boards" v-for="board in boards" :key="board.name">ALL BOARDS ({{board.length}})</p>
  ...
  <div class="end">
    ...
    <button class="hide-sidebar" @click="toggleSidebar()"><img src="../assets/icon-hide-sidebar.svg" alt=""> Hide Sidebar</button>
  </div>
  </div>
  <button v-if="isSidebarOpen === false" @click="toggleSidebar()" class="show-sidebar"><img src="../assets/icon-show-sidebar.svg" alt=""></button>
  </div>
</template>
<script>
...
nav {
  ...
  height: 97px;
  background: $DarkGrey;
  border-bottom: 1px solid $LinesDark;
  display: flex;
  z-index: 1;
  position: fixed;
  ...
  .logo {
    width: 210px;
    border-right: $LinesDark 1px solid;
    height: 100%;
    display: flex;
...
height: 100%;
 background: $DarkGrey;
  border-right: 1px solid $LinesDark;
  display: flex;
  justify-content: space-between;
